...
home = os.getenv('HOME')
fit_results_dir = home + '/Documents/sn_sit_hackday/'
...
def main():
    ...
    for fl in glob.glob(fit_results_dir + '*.h5'):
        ...
        if flsize < 33:  # full sampler size is 33.6 MB 
        ...
        z_acc_sn.append(zacc)

        zerr_up = cq_z[2] - cq_z[1]
        zerr_low = cq_z[1] - cq_z[0]

        z_err_sn.append([zerr_low, zerr_up])
    ...
    print("Catastrophic failures:", np.sort(fails))
    ...
    z_acc_sn = np.array(z_acc_sn)
    z_err_sn = np.array(z_err_sn)
    z_truth = np.array(z_truth)

    snr_arr_sn = np.array(snr_arr_sn)
    ...
    ax1.errorbar(snr_arr_sn, z_acc_sn, yerr=z_err_sn, fmt='o', \
    ...
    zoom_idx = np.where(np.abs(z_acc_sn) <= 0.01)[0]
    print("Num points within 1 percent accuracy:", len(zoom_idx))

    ax_inset.errorbar(snr_arr_sn[zoom_idx], z_acc_sn[zoom_idx], yerr=z_err_sn[:, zoom_idx], fmt='o', \
        markersize=2.0, markerfacecolor='k', markeredgecolor='k', ecolor='k', elinewidth=0.3)
    ...
    fig.savefig('snr_vs_z_accuracy_snanaSNsim.pdf', dpi=300, bbox_inches='tight')
    ...
    ax1.errorbar(z_truth, z_corner, yerr=z_err_sn, fmt='o', \
    ...
    ax2.errorbar(z_truth, z_acc_sn, yerr=z_err_sn, fmt='o', \
        markersize=2.0, markerfacecolor='k', markeredgecolor='k', \
        ecolor='k', elinewidth=0.3)
    ...
    fig1.savefig('redshift_acc_comparison_snanaSNsim.pdf', dpi=300, bbox_inches='tight')
    ...
    fail_idx = np.where(np.abs(z_acc_sn) >= 0.1)[0]
    ...
    fig2.savefig('fails_hist_snr.pdf', dpi=300, bbox_inches='tight')
    ...
    fig3.savefig('phase_recovery_snr.pdf', dpi=300, bbox_inches='tight')
